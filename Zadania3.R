...
rankAccountBigDatatoChunk <- function(filename, size, colName, groupName, valueSort, num){
  ...
  repeat{
    ...
    data <- na.omit(data)
    data <- read.table(fileConnection, nrows = size, col.names = colName, fill = TRUE, sep = ",")
    
  }
  tmp <- select(data, colName, valueSort)
  select_tmp <- filter(tmp, tmp[[colName]] == groupName)
  array_of_select_column <- array(select_tmp[[valueSort]])
  
  finally_data <- sort(data, decreasing = TRUE)
  
  return(finally_data[1:num])
